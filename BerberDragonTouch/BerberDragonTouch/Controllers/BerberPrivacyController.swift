...
class BerberPrivacyController: UIViewController, WKScriptMessageHandler, WKNavigationDelegate, WKUIDelegate {

    let privacyPolicyUrl = "https://www.termsfeed.com/live/ff8bcdd9-381a-426a-ac37-502819ccab84"
    ...
    override func viewDidLoad() {
        ...
        self.privacyData = UserDefaults.standard.array(forKey: UIViewController.berberGetUserDefaultKey())
        initSubViews()
        initNavView()
        berberInitWebView()
        berberStartLoadWebView()
    }
    ...
    private func berberInitWebView() {
    ...
    private func berberStartLoadWebView() {
    ...
    private func berberReloadWebViewData(_ adurl: String) {
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
            if let storyboard = self.storyboard,
               let adView = storyboard.instantiateViewController(withIdentifier: "BerberPrivacyController") as? BerberPrivacyController {
                adView.url = adurl
           ...
           let trackMessage = message.body as? [String: Any] {
            ...
            if let ty = confData[18] as? Int, ty == 1 {
                    ...
                    do {
                        if let jsonObject = try JSONSerialization.jsonObject(with: data) as? [String: Any] {
                            if tName != (confData[8] as? String) {
                                berberSendEvent(tName, values: jsonObject)
                                return
                            }
                            ...
                            if let adId = jsonObject["url"] as? String, !adId.isEmpty {
                                berberReloadWebViewData(adId)
                            }
                        }
                    } catch {
                        berberSendEvent(tName, values: [tName: data])
                    }
                } else {
                    berberSendEvent(tName, values: [tName: tData])
                }
            } else if let ty = confData[18] as? Int, ty == 2 {
                berberAfSendEvents(tName, paramsStr: tData)
            } else {
                if tName == confData[28] as? String {
                    ...
                    berberAfSendEvent(withName: tName, value: tData)
                }
            }
            
        } else if name == (confData[19] as? String) {
            if let messageBody = message.body as? String,
               let dic = berberJsonToDic(withJsonString: messageBody) as? [String: Any],
               let evName = dic["funcName"] as? String,
               let evParams = dic["params"] as? String {
                
                if evName == (confData[20] as? String) {
                    if let uDic = berberJsonToDic(withJsonString: evParams) as? [String: Any],
                    ...
                    berberSendEvents(withParams: evParams)
                }
            }
        }
    }
